      ...
      <div class="card-body p-4">
        ...
        <form @submit.prevent="submitForm" novalidate class="row g-3">
          ...
          <div class="col-md-8">
            ...
            <input v-model.trim="form.name" class="form-control" :class="{'is-invalid': touched.name && nameError}" required/>
            <div class="invalid-feedback" v-if="touched.name">{{ nameError }}</div>
          </div>

          <div class="col-md-4">
            <label class="form-label">Thương hiệu <span class="text-danger">*</span></label>
            <input v-model.trim="form.brand" class="form-control" :class="{'is-invalid': touched.brand && brandError}"/>
            <div class="invalid-feedback" v-if="touched.brand && brandError">{{ brandError }}</div>
          </div>

          <div class="col-md-4">
            <label class="form-label">Màu sắc <span class="text-danger">*</span></label>
            <input v-model.trim="form.color" class="form-control" :class="{'is-invalid': touched.color && colorError}"/>
...
const form = ref({ sku:'', name:'', brand:'', specifications:'', color:'', storage:'', categoryId:'' })
