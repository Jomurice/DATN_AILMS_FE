...
async function drawChart () {
  ...
  const cfg = {
    ...
    options: {
      ...
      plugins: {
        legend: { position: 'top', onClick: () => {}, labels: { color: '#6b7280', font: { weight: 500 } } },
...
onMounted(async () => {
  try { warehouses.value = await warehouseService.getAll() || [] } catch {}
  await loadStats()
  await drawChart()
})
...
@media (max-width:992px){ .dbox{ flex-direction:column; } }
