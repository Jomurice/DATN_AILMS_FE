          ...
          <tbody>
            <tr v-for="w in items" :key="w.id">
              ...
              <td class="d-flex gap-2 flex-wrap">
                ...
                <router-link :to="`/warehouse/${w.id}/zone`" class="btn btn-sm btn-outline-secondary">Show Zones</router-link>
    ...
    <div v-else-if="ui==='detail'">
      ...
      <div class="card border-0 shadow-sm">
        <div class="card-body">
          ...
          <div class="mb-2"><small class="text-muted">Location</small><div>{{ detail?.location }}</div></div>

          <h6 class="fw-bold mt-4">Zones ({{ zones.length }})</h6>
          <div class="table-responsive">
            <table class="table table-sm">
              <thead><tr><th style="width:220px">ID</th><th>Tên</th><th>Mô tả</th><th></th></tr></thead>
              <tbody>
                <tr v-for="z in zones" :key="z.id">
                  ...
                  <td>{{ z.description }}</td>
                  <td>
                    <router-link class="btn btn-sm btn-outline-primary" :to="`/warehouse/${detail.id}/zone/${z.id}/aisle`">Manage Aisles</router-link>
                  </td>
                </tr>
...
<style scoped>
.warehouse-management{ padding:24px; background:#fff; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,.1) }
</style>
