      ...
      <main class="main">
        ...
        <div v-else-if="ui === 'detail'" class="section-card">
          ...
          <div class="card border-0 shadow-sm p-4">
            ...
            <div class="mb-2">
              <small class="text-muted">Dãy</small>
              <div>{{ path.aName || "—" }}</div>
            </div>
...
async function load() {
  items.value = (await shelfService.getByAisle(aid)) || [];
}
...
async function submit() {
  if (isEdit.value){
    ...
    await shelfService.update(editingId.value, { ...form.value, aisleId: aid });
  }
  else {
    const payload = {...form.value, aisleId: aid};
