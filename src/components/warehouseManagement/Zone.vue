            ...
            <form class="row g-3" @submit.prevent="submit">
              <div class="col-md-6">
                <label class="form-label"
                  >Tên khu <span class="text-danger">*</span></label
                >
                  ...
                  <tr v-for="a in aisles" :key="a.id">
                    ...
                    <td>
                      <RouterLink
                        ...
                        :to="`/warehouse/${wid}/zone/${detail.id}/aisle/${a.id}/shelf`"
                        >Quản lý kệ</RouterLink
                      >
                    </td>
                  </tr>
                  <tr v-if="!aisles.length">
...
async function load() {
  items.value = (await zoneService.getAll(wid)) || [];
}
async function openDetail(id) {
  detail.value = await zoneService.getById(id);
  aisles.value = (await aisleService.getAll(id)) || [];
  ui.value = "detail";
}
function openAdd() {
  isEdit.value = false;
...
function openEdit(z) {
  ...
  form.value = { name: z.name, code: z.code };
  ui.value = "form";
}
async function submit() {
  if (isEdit.value){ await zoneService.update(editingId.value, form.value);
  }else {
    const payload =  { ...form.value, warehouseId: wid };
