  ...
  <aside class="side">
    ...
    <nav class="tree">
      <div v-for="wh in tree" :key="wh.id" class="tree-node">
        ...
        <div
          class="tree-item"
          :class="{ active: selected === key(wh) }"
          @click="select(wh, `/warehouse/${wh.id}/zone`)"
        >
        ...
        <div v-show="isOpen(wh)" class="tree-children">
          <div v-for="z in wh.children" :key="z.id" class="tree-node">
            <div
              class="tree-item"
              :class="{ active: selected === key(z) }"
              @click="select(z, `/warehouse/${wh.id}/zone/${z.id}/aisle`)"
            >
            ...
            <div v-show="isOpen(z)" class="tree-children">
              <div v-for="a in z.children" :key="a.id" class="tree-node">
                <div
                  class="tree-item"
                  :class="{ active: selected === key(a) }"
                  @click="select(a, `/warehouse/${wh.id}/zone/${z.id}/aisle/${a.id}/shelf`)"
                >
                ...
                <div v-show="isOpen(a)" class="tree-children">
                  <div v-for="s in a.children" :key="s.id" class="tree-node">
                    <div
                      class="tree-item"
                      :class="{ active: selected === key(s) }"
                      @click="select(s, `/warehouse/${wh.id}/zone/${z.id}/aisle/${a.id}/shelf/${s.id}/bin`)"
                    >
                    ...
                    <div v-show="isOpen(s)" class="tree-children">
                      <div
                        ...
                        :class="{ active: selected === key(b) }"
                        @click="select(b, `/warehouse/${wh.id}/zone/${z.id}/aisle/${a.id}/shelf/${s.id}/bin/${b.id}`)"
                      >
