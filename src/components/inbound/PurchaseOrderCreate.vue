  ...
  <div class="page">
    ...
    <div class="card">
      <div class="grid">
        ...
        <div class="col">
          <label class="lbl">Người tạo (hiển thị)</label>
          <input v-model.trim="createdBy" class="ipt" placeholder="Người tạo" />
...
const form = ref({
  ...
  supplier: "",
  status: "PENDING",
  createdAt: todayStr(), // yyyy-MM-dd
  items: [], // { productId, orderQuantity, sku, name, categoryName, brandName, color }
});

const products = ref([]);              // từ API /api/products
...
function resolveUserId() {
  const u = getAuthUser();
  return u?.id || u?.userId || u?.uid || u?.sub || "";
}
function resolveUserName() {
  const u = getAuthUser();
  return u?.name || u?.fullName || u?.email || "";
}
...
function addLine() {
  ...
  if (ex) {
    const sum = (ex.orderQuantity || 0) + Number(qty.value);
    ex.orderQuantity = Math.min(9999, Math.max(1, sum));
  } else {
...
onMounted(() => {
  ...
  createdBy.value = resolveUserName();
  loadProducts();
});
</script>
