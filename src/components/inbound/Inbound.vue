            ...
            <div class="fw-bold">
              <div>
                ...
                <span class="text-muted">{{
                  cut(selectedOrder.supplier, 28)
                }}</span>
              </div>
              <small class="text-muted">
                Ngày tạo:
                {{ d(selectedOrder.createdAt || selectedOrder.eta) }} • Trạng
                thái:
...
//       // Gọi API lấy chi tiết đơn hàng + items + serials
//       full = await purchaseOrderService.getPurchaseOrderById(o.id, {
//         includeItems: true,
//       });
//     }

//     // Chuẩn hóa dữ liệu (cha -> con)
//     selectedOrder.value = normalizeOrder(full);

//     // ✅ khởi tạo scannedBySku từ dữ liệu con
//     scannedBySku.value = {};
...
async function openOrder(o) {
  try {
    // 🔥 luôn lấy lại chi tiết mới nhất từ backend (kể cả khi o đã có items)
    const full = await purchaseOrderService.getPurchaseOrderById(o.id, {
      includeItems: true,
    });

    selectedOrder.value = normalizeOrder(full);

    // ✅ reset lại danh sách serial đã quét theo SKU
    scannedBySku.value = {};
    for (const item of selectedOrder.value.items || []) {
      const skuKey = key(item.sku);
      scannedBySku.value[skuKey] = {
    ...
    // ✅ cập nhật lại danh sách orders.value để giữ dữ liệu đồng bộ
...
function normalizeOrder(order) {
  const items = (order.purchaseOrderItems || order.items || []).map((it) => {
    ...
    // tính toán số lượng quét
    const scannedCount = details.filter(
      (d) => d.status === "SCANNED" || d.status === "ACTIVE"
    ).length;
...
async function openSerialsModal(sku, productId) {
  ...
  try {
    ...
    const res = await api.get(
      `/api/purchase-orders/${selectedOrder.value.id}/serials`,
      {
        params: { sku },
      }
    );
...
onMounted(async () => {
  ...
  try {
    ...
    toast("Không tải được danh sách phiếu");
  } finally {
    loading.value = false;
  }
});
